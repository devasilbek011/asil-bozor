import { useTranslation } from "react-i18next";
import { FaArrowUp } from "react-icons/fa";

export default function Home() {
    const { t, i18n } = useTranslation();
  return (
    <>
    
    <div className="pastgi_qismkattasi">
        <div className="past_card">
          <img src="./assets/naushniklar/naushnik14.jpg" alt="" />
          <br />
          <h4>Airpods A9Pro</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>388.000</s> <br />
          <div className="flex">
            <h4 className="font">165.000 </h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/atirlar/atir8.jpg" alt="" />
          <br />
          <h4>MAGIC NOIRE</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>350.000</s> <br />
          <div className="flex">
            <h4 className="font">99.000 </h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/smart_s/smarts1.jpg" alt="" />
          <br />
          <h4>Smart soat SP8   </h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>530.000</s> <br />
          <div className="flex">
            <h4 className="font">199.000 </h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/naushniklar/naushnik6.jpg" alt="" />
          <br />
          <h4>BOROFONE BM83</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>65.000</s> <br />
          <div className="flex">
            <h4 className="font">25.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>
      </div>

      <div className="pastgi_qismkattasi">
        <div className="past_card">
          <img src="./assets/texnika/texnika1.jpg" alt="" />
          <br />
          <h4>LIDER, 2KW PECHKA,</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>450.000</s> <br />
          <div className="flex">
            <h4 className="font">320.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Telefonlar/phone6.jpg" alt="" />
          <br />
          <h4>Poco M6 Pro</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>4.550.000</s> <br />
          <div className="flex">
            <h4 className="font">2.999.000 </h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Mashinan/Mashinan1.jpg" alt="" />
          <br />
          <h4>Areon, het taratgich</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>140.000</s> <br />
          <div className="flex">
            <h4 className="font">95.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Telefonlar/phone1.jpg" alt="" />
          <br />
          <h4>GIONEE E16+</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>350.000</s> <br />
          <div className="flex">
            <h4 className="font">190.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>
      </div>

      <div className="pastgi_qismkattasi">
        <div className="past_card">
          <img src="./assets/atirlar/Atir1.jpg" alt="" />
          <br />
          <h4>YARA Atir</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>260.000</s> <br />
          <div className="flex">
            <h4 className="font">48.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Mashinan/Mashinan2.jpg" alt="" />
          <br />
          <h4>PERODNIK 8 PIN TO 3.5,</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>120.000</s> <br />
          <div className="flex">
            <h4 className="font">28.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/atirlar/atir2.jpg" alt="" />
          <br />
          <h4>Flora Atir</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>260.000</s> <br />
          <div className="flex">
            <h4 className="font">48.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/naushniklar/naushnik15.jpg" alt="" />
          <br />
          <h4>Airpods Inkax T02 Black</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>480.000</s> <br />
          <div className="flex">
            <h4 className="font">160.000 </h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>
      </div>

      <div className="pastgi_qismkattasi">
        <div className="past_card">
          <img src="./assets/atirlar/atir3.jpg" alt="" />
          <br />


          <h4>HAYAATI, Atir</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>320.000</s> <br />
          <div className="flex">
            <h4 className="font">48.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Mashinan/Mashinan3.jpg" alt="" />
          <br />
          <h4>Aroma Loop, het taratgich</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>180.000</s> <br />
          <div className="flex">
            <h4 className="font">48.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Telefonlar/phone16.jpg" alt="" />
          <br />
          <h4>Infinix Hot 30i</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>3.400.000</s> <br />
          <div className="flex">
            <h4 className="font">1.299.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Zarayatchik/zarayatchik1.jpg" alt="" />
          <br />
          <h4>65W GALOFKA</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>650.000</s> <br />
          <div className="flex">
            <h4 className="font">105.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>
      </div>


      <div className="pastgi_qismkattasi">
        <div className="past_card">
          <img src="./assets/Zarayatchik/zarayatchik3.jpg" alt="" />
          <br />
          <h4>67W GALOFKA</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>250.000</s> <br />
          <div className="flex">
            <h4 className="font">55.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/kalonkalar/Kalonka1.jpg" alt="" />
          <br />
          <h4>KALONKA BLUETOOTH SOONBOX S7,</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>380.000</s> <br />
          <div className="flex">
            <h4 className="font">150.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Telefonlar/phone20.jpg" alt="" />
          <br />
          <h4>Infinix Hot 40i</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>3.220.000</s> <br />
          <div className="flex">
            <h4 className="font">1.599.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Telefonlar/phone21.jpg" alt="" />
          <br />
          <h4>KETEL OCHILIB YOPILGICH TELEFON</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>750.000</s> <br />
          <div className="flex">
            <h4 className="font">379.000 </h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>
      </div>


      <div className="pastgi_qismkattasi">
        <div className="past_card">
          <img src="./assets/naushniklar/naushnik1.jpg" alt="" />
          <br />
          <h4>A8, Naushnik




          </h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>158.000</s> <br />
          <div className="flex">
            <h4 className="font">28.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/atirlar/Atir4.jpg" alt="" />
          <br />
          <h4>No.2 MEN Atir</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>320.000</s> <br />
          <div className="flex">
            <h4 className="font">48.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

       

        <div className="past_card">
          <img src="./assets/naushniklar/naushnik2.jpg" alt="" />
          <br />
          <h4>i16 Max airpods</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>280.000</s> <br />
          <div className="flex">
            <h4 className="font">55.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Telefonlar/phone25.jpg" alt="" />
          <br />
          <h4>Samsung A16</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>3.800.000</s> <br />
          <div className="flex">
            <h4 className="font">2.000.000 </h4>
            <h4 className="tf">{t("title10")}</h4>
          
          </div>
        </div>
      </div>


      <div className="pastgi_qismkattasi">
        <div className="past_card">
          <img src="./assets/naushniklar/naushnik3.jpg" alt="" />
          <br />
          <h4>i11 Airpods</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>280.000</s> <br />
          <div className="flex">
            <h4 className="font">55.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/atirlar/Atir5.jpg" alt="" />
          <br />
          <h4>LA VAID EAST BELLAH</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>260.000</s> <br />
          <div className="flex">
            <h4 className="font">48.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Telefonlar/phone2.jpg" alt="" />
          <br />
          <h4>Honor X5 Plus</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>2.000.000</s> <br />
          <div className="flex">
            <h4 className="font">1.599.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Telefonlar/phone3.jpg" alt="" />
          <br />
          <h4>Honor X8a</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>3.500.000</s> <br />
          <div className="flex">
            <h4 className="font">2.899.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          
          </div>
        </div>
      </div>

      <div className="pastgi_qismkattasi">
        <div className="past_card">
          <img src="./assets/naushniklar/naushnik7.jpg" alt="" />
          <br />
          <h4>Naushnik D-37</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>125.000</s> <br />
          <div className="flex">
            <h4 className="font">28.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/atirlar/Atir7.jpg" alt="" />
          <br />
          <h4>NORKOTIQUE ATIR</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>560.000</s> <br />
          <div className="flex">
            <h4 className="font">220.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Telefonlar/phone7.jpg" alt="" />
          <br />
          <h4>Realme Note 50</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>3.700.000</s> <br />
          <div className="flex">
            <h4 className="font">1.449.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Telefonlar/phone8.jpg" alt="" />
          <br />
          <h4>Redmi 9A</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>2.100.000</s> <br />
          <div className="flex">
            <h4 className="font">999.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          
          </div>
        </div>
      </div>

      <div className="pastgi_qismkattasi">
        <div className="past_card">
          <img src="./assets/naushniklar/naushnik8.jpg" alt="" />
          <br />
          <h4>Naushnik Gfuz HF-16</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>250.000</s> <br />
          <div className="flex">
            <h4 className="font">65.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/atirlar/Atir6.jpg" alt="" />
          <br />
          <h4>Marque Atir</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>430.000</s> <br />
          <div className="flex">
            <h4 className="font">99.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Telefonlar/phone4.jpg" alt="" />
          <br />
          <h4>Honor X9c Smart</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>4.500.000</s> <br />
          <div className="flex">
            <h4 className="font">3.899.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Telefonlar/phone5.jpg" alt="" />
          <br />
          <h4>Novey 105</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>450.000</s> <br />
          <div className="flex">
            <h4 className="font">219.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          
          </div>
        </div>
      </div>

      <div className="pastgi_qismkattasi">
        <div className="past_card">
          <img src="./assets/naushniklar/naushnik9.jpg" alt="" />
          <br />
          <h4>Naushnik MI YDJC01</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>180.000</s> <br />
          <div className="flex">
            <h4 className="font">28.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/atirlar/Atir9.jpg" alt="" />
          <br />
          <h4>Barakkat Rouge 540</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>600.000</s> <br />
          <div className="flex">
            <h4 className="font">220.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Telefonlar/phone9.jpg" alt="" />
          <br />
          <h4>Redmi 12</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>3.200.000</s> <br />
          <div className="flex">
            <h4 className="font">1.599.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Telefonlar/phone10.jpg" alt="" />
          <br />
          <h4>Redmi 12C</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>2.800.000</s> <br />
          <div className="flex">
            <h4 className="font">1.699.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          
          </div>
        </div>
      </div>

      <div className="pastgi_qismkattasi">
        <div className="past_card">
          <img src="./assets/naushniklar/naushnik10.jpg" alt="" />
          <br />
          <h4>Naushnik Oraimo</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>80.000</s> <br />
          <div className="flex">
            <h4 className="font">22.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/atirlar/Atir10.jpg" alt="" />
          <br />
          <h4>DUBAI FAKHAR ATIR</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>800.000</s> <br />
          <div className="flex">
            <h4 className="font">450.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Telefonlar/phone11.jpg" alt="" />
          <br />
          <h4>Redmi Note 12 Pro</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>3.640.000</s> <br />
          <div className="flex">
            <h4 className="font">1.449.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Telefonlar/phone12.jpg" alt="" />
          <br />
          <h4>Redmi Note 13</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>4.290.000</s> <br />
          <div className="flex">
            <h4 className="font">2.500.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          
          </div>
        </div>
      </div>

      <div className="pastgi_qismkattasi">
        <div className="past_card">
          <img src="./assets/naushniklar/naushnik11.jpg" alt="" />
          <br />
          <h4>Naushnik R-5</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>100.000</s> <br />
          <div className="flex">
            <h4 className="font">22.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/atirlar/Atir11.jpg" alt="" />
          <br />
          <h4>DUBAI FAKHAR ATIR</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>800.000</s> <br />
          <div className="flex">
            <h4 className="font">450.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Telefonlar/phone15.jpg" alt="" />
          <br />
          <h4>Infinix Smart 7 plus</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>2.260.000</s> <br />
          <div className="flex">
            <h4 className="font">1.199.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Telefonlar/phone14.jpg" alt="" />
          <br />
          <h4>i1 series</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>2.520.000</s> <br />
          <div className="flex">
            <h4 className="font">1.400.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          
          </div>
        </div>
      </div>

      <div className="pastgi_qismkattasi">
        <div className="past_card">
          <img src="./assets/naushniklar/naushnik12.jpg" alt="" />
          <br />
          <h4>STEREO EARPHON NAUSHNIK</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>65.000</s> <br />
          <div className="flex">
            <h4 className="font">12.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/atirlar/Atir12.jpg" alt="" />
          <br />
          <h4>INTENSE MAN</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>500.000</s> <br />
          <div className="flex">
            <h4 className="font">280.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Telefonlar/phone18.jpg" alt="" />
          <br />
          <h4>Infinix Smart 8</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>2.380.000</s> <br />
          <div className="flex">
            <h4 className="font">1.099.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Telefonlar/phone17.jpg" alt="" />
          <br />
          <h4>Infinix Smart 7</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>3.650.000</s> <br />
          <div className="flex">
            <h4 className="font">1.099.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          
          </div>
        </div>
      </div>

      <div className="pastgi_qismkattasi">
        <div className="past_card">
          <img src="./assets/naushniklar/naushnik13.jpg" alt="" />
          <br />
          <h4>Naushnik TINGWODE S1</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>160.000</s> <br />
          <div className="flex">
            <h4 className="font">32.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/naushniklar/naushnik16.jpg" alt="" />
          <br />
          <h4>JBL byHARMAN 881A quloqchin</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>450.000</s> <br />
          <div className="flex">
            <h4 className="font">150.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Telefonlar/phone19.jpg" alt="" />
          <br />
          <h4>Infinx Hot 30 play</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>2.430.000</s> <br />
          <div className="flex">
            <h4 className="font">1.499.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Telefonlar/phone22.jpg" alt="" />
          <br />
          <h4>Samsung A14</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>3.685.000</s> <br />
          <div className="flex">
            <h4 className="font">1.799.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          
          </div>
        </div>
      </div>

      <div className="pastgi_qismkattasi">
        <div className="past_card">
          <img src="./assets/Telefonlar/phone23.jpg" alt="" />
          <br />
          <h4>Samsung A15</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>4.200.000</s> <br />
          <div className="flex">
            <h4 className="font">1.999.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Telefonlar/phone24.jpg" alt="" />
          <br />
          <h4>Samsung A06</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>3.210.000</s> <br />
          <div className="flex">
            <h4 className="font">1.449.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Telefonlar/phone27.jpg" alt="" />
          <br />
          <h4>Tecno Spark Go</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>2.350.000</s> <br />
          <div className="flex">
            <h4 className="font">999.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Telefonlar/phone26.jpg" alt="" />
          <br />
          <h4>Tecno Spark 20c</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>2.185.000</s> <br />
          <div className="flex">
            <h4 className="font">1.499.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          
          </div>
        </div>
      </div>

      <div className="pastgi_qismkattasi">
        <div className="past_card">
          <img src="./assets/Telefonlar/phone28.jpg" alt="" />
          <br />
          <h4>Texno Max 025</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>850.000</s> <br />
          <div className="flex">
            <h4 className="font">380.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Telefonlar/phone30.jpg" alt="" />
          <br />
          <h4>TEXNO MAX22</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>780.000</s> <br />
          <div className="flex">
            <h4 className="font">379.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Telefonlar/phone31.jpg" alt="" />
          <br />
          <h4>Iphone 13 Pro</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>600$</s> <br />
          <div className="flex">
            <h4 className="font">470 $</h4>
           
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Telefonlar/phone32.jpg" alt="" />
          <br />
          <h4>Iphone 11</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>500 $</s> <br />
          <div className="flex">
            <h4 className="font">240 $</h4>
           
          
          </div>
        </div>
      </div>

      <div className="pastgi_qismkattasi">
        <div className="past_card">
          <img src="./assets/Telefonlar/phone33.jpg" alt="" />
          <br />
          <h4>Samsung A55 5G</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>8.700.000</s> <br />
          <div className="flex">
            <h4 className="font">4.550.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Telefonlar/phone34.jpg" alt="" />
          <br />
          <h4>Samsung A35 5G</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>5.480.000</s> <br />
          <div className="flex">
            <h4 className="font">3.800.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Telefonlar/phone35.jpg" alt="" />
          <br />
          <h4>Honor X5b plus</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>3.900.000</s> <br />
          <div className="flex">
            <h4 className="font">1.900.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Telefonlar/phone36.jpg" alt="" />
          <br />
          <h4>Honor X6b</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>5.200.000</s> <br />
          <div className="flex">
            <h4 className="font">2.300.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          
          </div>
        </div>
      </div>

      <div className="pastgi_qismkattasi">
        <div className="past_card">
          <img src="./assets/Telefonlar/phone37.jpg" alt="" />
          <br />
          <h4>Honor X8b</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>
          6.300.000</s> <br />
          <div className="flex">
            <h4 className="font">3.300.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Telefonlar/phone38.jpg" alt="" />
          <br />
          <h4>vivo Y03</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>5.150.000</s> <br />
          <div className="flex">
            <h4 className="font">2.200.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Telefonlar/phone39.jpg" alt="" />
          <br />
          <h4>Honor X9b</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>6.000.000</s> <br />
          <div className="flex">
            <h4 className="font">4.150.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          </div>
        </div>

        <div className="past_card">
          <img src="./assets/Telefonlar/phone40.jpg" alt="" />
          <br />
          <h4>Redmi 14C</h4>
          <i className="i">{t("title11")}</i>
          <br />
          <br />
          <s>3.420.000</s> <br />
          <div className="flex">
            <h4 className="font">1.800.000</h4>
            <h4 className="tf">{t("title10")}</h4>
          
          </div>
        </div>
      </div>

      <button className="fix_btn"><a href="#home"><FaArrowUp className="ico" /></a></button>
      
    </>
  );
}
